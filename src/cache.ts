...
export class CacheManager {
  ...
  private initialize(): void {
    try {
      ...
      logger.warn(`Failed to initialize cache: ${formatError(error)}`);
      this.enabled = false;
    }
  }
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          logger.debug(`Could not get stats for ${filePath}: ${formatError(error)}`);
        }
      }
    } catch (error: any) {
      logger.warn(`Failed to calculate cache size: ${formatError(error)}`);
    }
  }
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          logger.debug(`Could not process ${filePath}: ${formatError(error)}`);
        }
      }
    } catch (error: any) {
      logger.warn(`Failed to clean expired entries: ${formatError(error)}`);
    }
    ...
    try {
      ...
      while (this.currentSize > this.maxSize * 0.8 && index < files.length) { // Aim to reduce to 80% of max
        ...
        try {
          ...
          logger.debug(`Could not remove ${filePath}: ${formatError(error)}`);
        }
      }
    } catch (error: any) {
      logger.warn(`Failed to enforce max cache size: ${formatError(error)}`);
    }
  }
  ...
  get<T>(key: string, keyPrefix?: string): T | null {
    ...
    try {
      ...
      logger.debug(`Cache get error for ${fullKey}: ${formatError(error)}`);
    }
  ...
  set<T>(key: string, data: T, keyPrefix?: string): void {
    ...
    try {
      ...
      logger.debug(`Cache set error for ${fullKey}: ${formatError(error)}`);
    }
  }
    ...
    try {
      ...
      for (const file of files) {
        try {
          ...
          logger.debug(`Could not delete ${file}: ${formatError(error)}`);
        }
      }
      ...
      logger.warn(`Failed to clear cache: ${formatError(error)}`);
    }
  }
  ...
  clearByPrefix(prefix: string): void {
    ...
    try {
      ...
      logger.warn(`Failed to clear cache by prefix: ${formatError(error)}`);
    }
  }
  ...
  getStats(): { enabled: boolean; entries: number; sizeInMB: number; directory: string } {
    ...
    if (this.enabled && fs.existsSync(this.directory)) {
      try {
        ...
        logger.debug(`Could not count cache entries: ${formatError(error)}`);
      }
    }
