...
export class BundleAnalyzer {
  ...
  public async analyzeBundleFromStats(statsFilePath: string): Promise<BundleAnalysis> {
    try {
      const statsData = await fs.promises.readFile(statsFilePath, 'utf-8');
      const stats = JSON.parse(statsData);
      
      return this.analyzeBundleFromObject(stats);
    } catch (error) {
      logger.error(`Erro ao analisar arquivo de estatísticas: ${error}`);
      throw error;
    }
  }

  public analyzeBundleFromObject(stats: any): BundleAnalysis {
    // Extrair módulos
    if (stats.modules) {
      this.analysis.modules = stats.modules.map((mod: any) => ({
        id: mod.id,
        name: mod.name,
        size: mod.size || 0,
    ...
    // Extrair chunks
    if (stats.chunks) {
      this.analysis.chunks = stats.chunks.map((chunk: any) => ({
        id: chunk.id,
  ...
  public async generateReport(outputPath: string): Promise<void> {
    try {
      ...
      logger.success(`Relatório de análise gerado em:\n${outputPath}\n${htmlPath}`);
    } catch (error) {
  ...
  private generateHTMLReport(): string {
    ...
    // Dados para gráficos
    const moduleSizes = JSON.stringify(
      largestModules.map(mod => ({
        name: path.basename(mod.name),
        size: mod.size,
        percentage: (mod.size / modulesTotalSize) * 100
      }))
    );
    
    const chunkSizes = JSON.stringify(
